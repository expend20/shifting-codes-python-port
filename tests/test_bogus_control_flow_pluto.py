"""Tests for the Pluto Bogus Control Flow pass."""
...
def test_pluto_bcf_adds_bogus_blocks(ctx, rng):
    ...
    with ctx.create_module("test") as mod:
        ...
        p = PlutoBogusControlFlowPass(rng=rng)
...
def test_pluto_bcf_transforms_unconditional_branches(ctx, rng):
...
def test_pluto_bcf_creates_valid_opaque_predicate(ctx, rng):
...
def test_pluto_bcf_deterministic(ctx):
    """Same seed should produce same BCF output."""
    results = []
    for _ in range(2):
        with ctx.create_module("test") as mod:
            make_branch_function(ctx, mod)
            p = PlutoBogusControlFlowPass(rng=CryptoRandom(seed=77))
