"""Tests for the Pluto Control Flow Flattening pass."""
...
def test_pluto_flattening_diamond(ctx, rng):
...
def test_pluto_flattening_loop(ctx, rng):
...
def test_pluto_flattening_single_block_noop(ctx, rng):
...
def test_pluto_flattening_deterministic(ctx):
    """Same seed should produce same flattened output."""
    results = []
    for _ in range(2):
        with ctx.create_module("test") as mod:
            make_branch_function(ctx, mod)
            p = PlutoFlatteningPass(rng=CryptoRandom(seed=55))
...
def test_pluto_flattening_no_key_array(ctx, rng):
    """Pluto variant should NOT have encrypted state (no key array or update_key)."""
    with ctx.create_module("test") as mod:
        make_branch_function(ctx, mod)
        func = mod.get_function("branch_func")
        ...
        ir = mod.to_string()
        assert "cff.keys" not in ir
        assert "__cff_update_key" not in ir
