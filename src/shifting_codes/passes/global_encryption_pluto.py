"""[Pluto] Global Encryption Pass — port of Pluto GlobalEncryption.cpp.
...
Unlike the Polaris variant, this version does NOT check is_global_constant
and uses a single inline XOR decrypt at the first function entry point.
"""
...
class PlutoGlobalEncryptionPass(ModulePass):
    ...
    @classmethod
    def _should_skip(cls, gv) -> bool:
        if gv.name.startswith("llvm."):
            return True
        if gv.linkage not in cls._ENCRYPTABLE_LINKAGES:
            return True
        vtype = gv.global_value_type
        if vtype.kind == llvm.TypeKind.Integer:
            return False
